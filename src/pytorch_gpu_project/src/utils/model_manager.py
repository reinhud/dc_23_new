...
class ModelManager:
    ...
    def _get_train_runs_model(self, model_name: str):
        ...
        train_runs = []
        if os.path.exists(f"output/training/{model_name}"):
            search_path = f"output/training/{model_name}"
            for train_run in self._listdir_fullpath(search_path):
                train_runs.append((model_name, f"{train_run}"))
        else:
            print("No training runs found for this model.")
        return train_runs

    def _get_train_runs_all(self):
        """Find the training runs for all models"""
        train_runs = []
        for model_name in os.listdir("output/training"):
            model_path = os.path.join("output/training", model_name)
    ...
    def _get_best_metric_from_df(self, metric: pd.DataFrame, watch_metric: str, greater_is_better: bool):
        if watch_metric not in metric.columns:
            raise RuntimeError(f"Watch metric {watch_metric} not tracked.")
        ...
        for model_name, train_run_path in train_runs:
            ...
            if greater_is_better:
                ...
                if best_model_info["best_metric"] is None or best_metric_run > best_model_info["best_metric"]:
                    ...
                    best_model_info["state_dict_path"] = f"src/{train_run_path}/best_model.pt"
                    best_model_info["train_history_path"] = f"src/{train_history_path}"
                    best_model_info["train_args_path"] = f"src/{train_run_path}/train_args.json"
            else:
