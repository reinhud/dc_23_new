...
class TimmInferer:
    ...
    def infer(self, model, images, topk: None):
        ...
        if topk:
            ...
            for proba in probabilities:
                ...
                pred = [{"label": labels[i.item()], "score": round(v.item(), 4)} for i, v in zip(i, v)]
                predictions.append(pred)
        else:
            for proba in probabilities:
                v, i = torch.max(proba, dim=0)
                pred = [{"label": labels[i.item()], "score": round(v.item(), 4)}]
                predictions.append(pred)
    ...
    def infer_from_file(self, file_path, model, topk: False):
        """Infer on a single picture."""
        if not os.path.exists(file_path):
            raise RuntimeError(f"File: {file_path} does not exist.")
        else:
            images = [Image.open(file_path) for img_path in os.listdir(file_path)]
